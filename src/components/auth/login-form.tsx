...
export function LoginForm() {
  ...
  const redirectPath = searchParams.get('redirect') || '/today';
  ...
  const onSubmit = async (data: LoginFormValues) => {
    try {
      ...
      toast.success('Welcome back!', {
        description: 'You have successfully signed in to Chronicle.',
      });
      router.push(redirectPath);
    } catch (error: any) {
      console.error(error);
      const friendlyError = error?.code === 'auth/invalid-credential'
      ...
      <Card className="border border-black/[0.06] dark:border-white/[0.06] bg-card/60 backdrop-blur-xl shadow-xl shadow-black/[0.03] rounded-2xl overflow-hidden relative">
        ...
        <CardContent className="px-7 pb-4">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <Button
              type="submit"
              className="w-full rounded-xl bg-gradient-to-tr from-violet-600 to-indigo-500 hover:from-violet-700 hover:to-indigo-600 text-white font-semibold shadow-md shadow-violet-500/10 transition-all gap-2 py-5"
              disabled={isSubmitting || isGoogleLoading}
            >
              {isSubmitting ? (
